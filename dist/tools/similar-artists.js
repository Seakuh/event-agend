...
export class SimilarArtists {
    async find(artistName, limit = 10) {
    ...
    async searchLastFm(artistName) {
        try {
            ...
            $(".similar-artists-item, .grid-items-item").each((_, el) => {
                const $el = $(el);
                const name = $el.find(".grid-items-item-main-text, .link-block-target").text().trim() ||
    ...
    async searchMusicBrainz(artistName) {
        try {
            ...
            const tags = artistResponse.data.tags?.map((t) => t.name) || [];
            const similar = relations
