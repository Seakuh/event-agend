            ...
            case "add_artist":
                return {
                    content: [
                        {
                            type: "text",
                            text: await artistManager.addArtist(args?.name, args?.type, args?.genres),
                        },
                    ],
                };
            case "remove_artist":
                return {
                    content: [
                        {
                            type: "text",
                            text: await artistManager.removeArtist(args?.name),
                        },
                    ],
                };
            case "list_artists":
                return {
                    content: [
                        {
                            type: "text",
                            text: await artistManager.listArtists(args?.type || "all"),
                        },
                    ],
                };
            case "search_events":
            ...
            case "find_similar_artists":
                return {
                    content: [
                        {
                            type: "text",
                            text: await similarArtists.find(args?.artist_name, args?.limit || 10),
                        },
                    ],
                };
            case "scrape_event_page":
                return {
                    content: [
                        {
                            type: "text",
                            text: await webScraper.scrape(args?.url, args?.search_terms),
                        },
                    ],
                };
            default:
