...
export class ArtistManager {
  ...
  private async loadData(): Promise<ArtistData> {
  ...
  private async saveData(data: ArtistData): Promise<void> {
    await this.ensureDataDir();
  ...
  async addArtist(
    ...
    const exists = data.artists.find(
  ...
  async removeArtist(name: string): Promise<string> {
    const data = await this.loadData();
